...
namespace VolumeTests
{
   ...
   public class POP3 : TestFixtureBase
   {
      ...
      {
         SingletonProvider<TestSetup>.Instance.AddAccount(_domain, "pop3@example.test", "test");
      }
      ...
      public void TestPOP3DownloadOfLargeMessage()
      {
         ...
         Pop3ClientSimulator.AssertMessageCount("pop3@example.test", "test", 1);
         ...
         socket.Send("USER " + "pop3@example.test" + "\r\n");
         socket.ReadUntil("+OK");
      ...
      public void Send100MBMessage()
      {
         ...
         socket.Send("MAIL FROM: pop3@example.test\r\n");
         socket.Receive();
         socket.Send("RCPT TO: pop3@example.test\r\n");
         socket.Receive();
         socket.Send("DATA\r\n");
         socket.Receive();
