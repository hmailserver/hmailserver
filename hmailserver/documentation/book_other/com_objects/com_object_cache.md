---
id: 386
title: "Cache object"
slug: com_object_cache
parent: com_objects
index: 0
is_book: false
updated: 2008-10-31
---

### Description

The cache object contains all settings related to hMailServer caching.

### Methods

<div class="api_method_name">Clear()</div>

<div class="api_description">Clears the cache and resets the statistics.</div>

### Properties

<div class="api_method_name">long AccountCacheTTL</div>

<div class="api_description">Account cache time (seconds).<br></div>

<div class="api_method_name">long AccountHitRate</div>

<div class="api_description">Account hit rate.<br> <i>(read-only)</i></div>

<div class="api_method_name">long AliasCacheTTL</div>

<div class="api_description">Alias cache time (seconds).<br></div>

<div class="api_method_name">long AliasHitRate</div>

<div class="api_description">Alias hit rate.<br> <i>(read-only)</i></div>

<div class="api_method_name">long DistributionListCacheTTL</div>

<div class="api_description">Distribution list cache time (seconds).<br></div>

<div class="api_method_name">long DistributionListHitRate</div>

<div class="api_description">Distribution list hit rate.<br> <i>(read-only)</i></div>

<div class="api_method_name">long DomainCacheTTL</div>

<div class="api_description">Domain cache time (seconds).<br></div>

<div class="api_method_name">long DomainHitRate</div>

<div class="api_description">Domain hit rate.<br> <i>(read-only)</i></div>

<div class="api_method_name">bool Enabled</div>

<div class="api_description">Cache is enabled.<br></div>
