...
This error message is issued if grey listing is enabled, the sender is empty, and the recipient and IP address (and empty sender) triplet does not match an existing greylisting triplet. This occurs after the SMTP 'data' transmission. The reason for this is that this may be a SMTP callback from another server that is veriying that the recipient exists, using the RCPT TO command.
...
If a SMTP client sends a SMTP command which hMailServer considers being too long, hMailServer issues this error. This error typically indicates a client defect or a hacking attempt.
