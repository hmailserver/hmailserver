...
namespace HM
{
   ...
   void
   MessageAttachmentStripper::WriteToDisk_(shared_ptr<Message> pMessage, MimeBody &oMainMessage, shared_ptr<MimeBody> pBody)
   {
      if (!pBody)
         return;
   ...
   bool
   MessageAttachmentStripper::IsGoodTextPart_(shared_ptr<MimeBody> pBody)
   {
      if (!pBody)
         return false;
