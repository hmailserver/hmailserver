   ...
   */
   ...
   public:
      ...
      enum Constants
      {
         ...
         // Below these, a cost parameter buys so little work that the hash is no
         // stronger than a salted SHA-256. Above the memory ceiling, a single
         // verification would allocate more than is reasonable to ask of a shared
      ...
      // either direction counts - lowering it is as deliberate as raising it. Also
      // true for anything this build cannot verify at all, such as a hash portion of
      // a length Compute_ never derives.
      ...
      // would never write ourselves - that is the only caller that passes
      // enforceLowerBoundChecks = false, since the RFC 6070 vectors fall below
      // OpenSSL's own SP800-132 floor (iterations >= 1000, salt >= 16 bytes,
      // key >= 14 bytes).
