...
namespace HM
{
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      const String originalMessageID = pMsgData->GetFieldValue("Message-ID");
      if (!originalMessageID.IsEmpty())
      {
         pNewMsgData->SetFieldValue("In-Reply-To", originalMessageID);
         pNewMsgData->SetFieldValue("References", originalMessageID);
      }
