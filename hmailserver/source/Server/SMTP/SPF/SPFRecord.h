...
namespace HM
{
   ...
   public:
      ...
      // The mechanism's target as written: a macro-string the expander of section 7
      // turns into a domain name at evaluation. Empty where the mechanism carried none,
      // which for a, mx and ptr means the domain being evaluated.
   ...
   // Parsed whole before any of it is acted on, and a term's target kept as written
   // - see README.md.
   class SPFRecord
   {
   public:
      ...
      // True if the text begins with the version section of section 4.5: "v=spf1",
      // compared without regard to case, then a space or the end of the record. Picks
      // the SPF record out of a domain's other TXT records.
      ...
      // The redirect modifier of section 6.1, used only when no mechanism matched. Kept
      // aside rather than among the directives because section 6 makes modifiers
   ...
   private:
      ...
      // One per shape of mechanism, because what each may carry after its name differs:
      // all nothing, include and exists a domain, a and mx a domain and either prefix
      // length, ptr a domain, ip4 and ip6 an address and the one length of its family.
