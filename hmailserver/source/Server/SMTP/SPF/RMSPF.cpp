...
#define SPFDEFTTL 1800 // 30 minutes
#endif //SPFCACHE
...
static lstrec** list; // list of fallbacks (if no cache file is used)
...
initspf(int multithreaded)
 {
  ...
  if (multithreaded>0) // not called by SPFInit()
   {
    ...
    hMutex=CreateMutex(NULL,false,"RMSPFMutex");
    if (hMutex==NULL)
      return false;
  ...
  hmodDnsApi=LoadLibrary("DNSAPI.DLL");
  if (hmodDnsApi==NULL)
   {
    initialized=-1;
    if (multithreaded>0)
...
#endif //RAMADB_LINKED
    isamflags=ISAM_KeyCompress|ISAM_RepairHeader;
...
checkversion(const char* s)
 {
  return memicmp(s,SPF_Version,SPF_VersionLen)==0
...
indomain(const char* subdomain, const char* domain)
 {
  ...
  if (i<0 || i>0 && subdomain[i-1]!='.')
    return false;
  if (memicmp(domain,subdomain+i,len)!=0)
    return false;
  return true;
 }
...
dnsquery(const char* domain, int type, DNS_RECORD** dnsrespp)
  ...
  *dnsrespp=NULL;
  ...
  if (rc>9999 || rc<9000 && rc!=ERROR_TIMEOUT) // not a DNS error
    return SPF_PermError;
  return SPF_TempError;
 }
...
static int
readlist(const char* key, int keylen, dnsrec** datapp, int datasize,
  time_t currtime)
 {
  int i;
  int datalen;
  ...
  for (;;) // (only once, trick to avoid 'goto')
   {
    ...
    if (lstp->lst_expire<currtime)
#endif //SPFCACHE
     {
      spffree(lstp);
      listn--;
      memmove(list+i,list+i+1,(listn-i)*sizeof(lstrec*));
      break;
     }
  ...
  for (;;) // (only once, trick to avoid 'goto')
   {
    ...
    if (listn>=listm)
     {
      listm+=256;
      listp=(lstrec**)spfrealloc(list,listm*sizeof(lstrec*));
      if (listp==NULL)
       {
	listm-=256;
	break;
    ...
    memmove(list+i+1,list+i,(listn-i)*sizeof(lstrec*));
    listn++;
  ...
  for (;;)
   {
	...
	if (dnsp->Flags.S.Section!=DNSREC_ADDITIONAL)
	  continue;
	if (stricmp(dnsp->pName,domain)!=0)
	  continue;
       }
      else // answer section
       {
...
// flags: 16= return host name even if not validated
//        8 = for 'ptr' mechanism or 'p' macro
  ...
  const char* parent, int flags, time_t currtime)
 {
  ...
  if (datalen<0)
  ...
  else
   {
	...
	for (i=0,cp=datap->dns_data; cp<cp1; i++,cp=strchr(cp,0)+1)
	 {
	  if (i<sizeof(done) && done[i]) // already checked
	    continue;
	  switch (bit)
	   {
	    case 4: // host name must be 'parent'
	      if (stricmp(cp,parent)!=0)
		continue;
	      break;
	  ...
	  if (k!=0) // don't check A record if not required
	   {
	    result=SPF_Fail;
	    break;
	   }
	  ...
	  result=-1;
	  if (i<sizeof(done))
	    done[i]=true; // don't check again
	 }
	if (result>=0)
	  break;
       }
      if (!(flags&16) || result==0)
	break;
      // try again without validating A records
     }
   }

  if (host!=NULL)
   {
    if (result!=0 && (!(flags&16) || result!=SPF_Fail))
      cp="Unknown"; // PTR not found or A/AAAA does not match IP address
    len=strlen(cp);
    if (len>hostsize-1)
      len=hostsize-1;
    memcpy(host,cp,len);
...
SPFGetHostName(int family, const void* ipaddr, char* host, int size,
  const char* domain)
 {
  ...
  return gethostname(ipv6,(const uchar*)ipaddr,host,size,domain,
    domain?16|4|2|1:16|1,time(NULL));
 }
...
parse_include(spfrec* spfp, const char** cpp, int prefix, const char* domain)
 {
  ...
  if (stricmp(dom,spfp->spf_domain)==0 || stricmp(dom,domain)==0) // loop
    return SPF_PermError;
	...
	for (i=0; i<mxcnt; i++)
	 {
	  for (k=mxcnt-1; k>i; k--)
	   {
	    if (dnslistp[k-1]->Data.MX.wPreference
	      >dnslistp[k]->Data.MX.wPreference)
	     {
	      dnsp=dnslistp[k-1];
  ...
  if (datalen<0)
  ...
  else
   {
	...
	break;
      ...
      result=findaddr(dnsresp,cp,spfp->spf_ipv6,spfp->spf_ipaddr,cidrlen,
	spfp->spf_time);
      if (result>=0)
       {
...
check_host(spfrec* spfp, const char* domain)
 {
  ...
  int result;
  int result1;
  int usewhitelist; // 1: use whitelist before 'all', 2: there was no SPF
  const dnsrec* datap;
  const char* cp;
  const char* name;
  char* redirect;
  ...
  while (result!=SPF_TempError && result!=SPF_PermError)
   {
	...
	break;
      ...
      // redirect
      if (namelen==8 && memicmp(name,"redirect",8)==0)
       {
    ...
    // all
    if (namelen==3 && memicmp(name,"all",3)==0)
     {
  ...
  if (result<0) // not yet determined
   {
	...
	if (usewhitelist && whitelist!=NULL)
     ...
     {
      if (stricmp(redirect,spfp->spf_domain)==0
	|| stricmp(redirect,domain)==0) // loop
	return SPF_PermError;
...
SPFQuery(int family, const void* ipaddr, const char* sender,
  ...
  const char** explain)
 {
  ...
  if (*spfdata.spf_domain=='['
    ...
    || memicmp(spfdata.spf_domain+1,"IPv6:",5)==0
