...
.DESCRIPTION
    The versions of OpenSSL, Boost, PostgreSQL (libpq) and MariaDB Connector/C that
    hMailServer builds against are not recorded in one place: they are embedded in the
    include/library paths of hMailServer.vcxproj and in the DLLs post-build.bat copies. This
    script reads them back out, so callers do not have to hard-code a version that then has
    to be remembered on every upgrade.
